...
function getColOfRating(rating) {
	...
	return ColorConverter.toStr(ColorConverter.toRGB(new HSV(50+(rating - 850) * 300 / 1650, 255,90)));
	if (rating < 1500) {
...
function getColOfScore(score) {
	...
	if (score == 0) {
		...
		return ColorConverter.toStr(ColorConverter.toRGB(new HSV(30 + (100-score) * 60 / 100, 100, 90)));
	}
}

function addAleph(text,r)
{
	var d=new Date();
	if(aprilFools()){
		var l=Math.ceil(Math.random()*5)+1,r=Math.ceil(Math.random()*5)+1;
		text += '<sup>';
		while(l) text += '&alefsym;',l--;
		text += "</sup>";
		text='</sup>'+text;
		while(r) text='&alefsym;'+text,r--;
		text='<sup>'+text;
	}
	else
		if (r >= 1600) {
			text += '<sup>';
			while(r>=1600) text += '&alefsym;',r-=200;
			text += "</sup>";
		}
		else if(r<=1400) {
			text='</sup>'+text;
			while(r<=1400) text='&alefsym;'+text,r+=100;
			text='<sup>'+text;
		}
	return text;
}
...
function getUserLink(username, rating, addSymbol) {
	...
	if (addSymbol) text=addAleph(text,rating);
